...
@bp.route("/update_datasource", methods=["POST"])
async def update_datasource():
    request_json = await request.get_json()
    updated_index_name = request_json.get("datasource", None)

    if not updated_index_name:
        return jsonify({"error": "datasource is required"}), 400

    try:
        # Update the settings with the new data source
        app_settings.datasource.update_index(updated_index_name)
        return jsonify({"message": f"Datasource updated successfully to {updated_index_name}"}), 200
    except Exception as e:
        logging.exception("Exception in /update_datasource")
        return jsonify({"error": str(e)}), 500
